...
export class TokensController {
    ...
    @Post("login")
    async login(@Body() loginDto: LoginDto, @Res({ passthrough: true }) response: Response): Promise<IAccessToken> {
    ...
    @Post("refresh")
    async refresh(@Req() request: Request, @Res({ passthrough: true }) response: Response): Promise<IAccessToken> {
        const refreshToken = request.cookies["Refresh-Token"]
        const { refresh, access } = await this.tokensService.refresh(refreshToken)
        this.setRefreshToken(refresh, response)
